...
public class BallManager : MonoBehaviour
{
    ...
    public bool checkAndExplodeSameColorBall(Ball bullet)
    {
        ...
        bool isExploded = listSameColors.Count >= 1;

        // check explode 
        while(listSameColors.Count >= 1)
        {
           // remove all same colors
            Ball ball = listSameColors[listSameColors.Count - 1].Ball;
            int ballNumber = ball.GetNumberBall(ball);
            int newBallNumber = 0; 
            if(bullet == null) listSameColors.Add(ball.GetGridPosition());     
            else listSameColors.Add(bullet.GetGridPosition());
            int noBallsSameColor = listSameColors.Count;
            foreach (GridCell cell in listSameColors)
            { 
               if (ball.Equals(cell.Ball)) continue;
                newBallNumber = ballNumber * 2;
                if (cell.Ball != null) cell.Ball.EffectExplodeBall();
                removeBallFromGrid(cell);
            }

            ball.number.text = newBallNumber.ToString();
            ball.SetBallColor(newBallNumber);
         
            int noBallFallingDown = removeAllUnHoldBall();

            if (_scoreEvent != null)
            {
                int calScore = _score.CalculateScore(noBallsSameColor, noBallFallingDown, ballNumber);
                _score.SetScore(_score.GetScore() + calScore);
                _scoreEvent(_score.GetScore());
            }
            listSameColors.Clear();
            listSameColors = _gridManager.GetListNeighborsSameColorRecursive(ball);       
        }
    ...
    public (float,float) PushDown()
    {
        float heightDown = _gridManager.GetCellSizeY();
        PivotGrid.localPosition -= new Vector3(0, heightDown * Time.deltaTime + speedPusher, 0); 
        return (heightDown,speedPusher);
    }

    public float PushUp()
    {
        float heightUp = _gridManager.GetCellSizeY();
        PivotGrid.localPosition += new Vector3(0, heightUp, 0);
        return heightUp;
    }
